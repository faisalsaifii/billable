<script lang="ts">
  ...
  interface StockRow { itemId: number; name: string; groupName: string; inQty: number; outQty: number; closingQty: number; }
  ...
  const filtered = $derived(search
    ? rows.filter(r => r.name.toLowerCase().includes(search.toLowerCase()) || r.groupName.toLowerCase().includes(search.toLowerCase()))
    : rows
  );
  let currentGroup = "";
</script>
...
<div class="p-6">
  <div class="flex justify-between items-center mb-4">
    ...
    <div class="flex gap-2">
      ...
      <button onclick={load} class="px-3 py-1.5 bg-neutral-600 hover:bg-neutral-500 rounded text-sm">Refresh</button>
    </div>
  </div>
        ...
        {#each filtered as row (row.itemId)}
          {#if row.groupName !== currentGroup}
            ...
            <tr class="bg-neutral-800 border-t border-gray-600">
              <td colspan="4" class="p-2 text-xs font-bold text-gray-400 uppercase">{row.groupName}</td>
            </tr>
          {/if}
          <tr class="border-b border-gray-700 hover:bg-neutral-800">
            ...
            <td class="p-2 text-right font-medium {row.closingQty < 0 ? 'text-red-300' : 'text-white'}">{row.closingQty.toFixed(2)}</td>
          </tr>
        {/each}
      </tbody>
    </table>
