<script lang="ts">
  ...
  interface TrialRow { accountId: number; name: string; groupName: string; debit: number; credit: number; }
        ...
        {#each rows as row (row.accountId)}
          {#if row.groupName !== currentGroup}
            ...
            <tr class="bg-neutral-800 border-t border-gray-600">
              <td colspan="3" class="p-2 text-xs font-bold text-gray-400 uppercase">{row.groupName}</td>
            </tr>
          {/if}
