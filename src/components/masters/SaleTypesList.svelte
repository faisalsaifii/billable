<script lang="ts">
  ...
  let formData = $state({
    ...
    taxInvoice: false, taxRate: 0, surchargeRate: 0, invoiceHeading: "",
  });

  const TAXATION_TYPES: SaleType["taxationType"][] = ["Taxable", "Exempt", "Tax Free", "Against ST Form", "Multi Tax", "Tax Paid"];

  const load = async () => {
    loading = true; error = "";
    try { await companyService.initialize(); saleTypes = await mastersService.getSaleTypes(companyId); }
  ...
  const handleSubmit = async () => {
    ...
    try {
      await mastersService.createSaleType({
        ...
        taxInvoice: formData.taxInvoice, taxRate: formData.taxRate,
        surchargeRate: formData.surchargeRate, invoiceHeading: formData.invoiceHeading || null,
        description: null,
      });
      formData = { name: "", alias: "", region: "Local", taxationType: "Taxable", taxInvoice: false, taxRate: 0, surchargeRate: 0, invoiceHeading: "" };
  ...
  const handleDelete = async (id: number) => {
    ...
    await mastersService.deleteSaleType(id); await load();
  };
    ...
    <div class="bg-neutral-800 border border-gray-600 rounded p-4 mb-6 max-w-lg">
      <div class="space-y-3">
        ...
        <label class="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" bind:checked={formData.taxInvoice} class="w-4 h-4" />
          <span class="text-sm">Tax Invoice (enables buyer Input Tax Credit)</span>
        </label>
        <Input name="invoiceHeading" label="Invoice Heading (optional)" type="text" bind:value={formData.invoiceHeading} />
      ...
      <tbody>
        {#each saleTypes as st (st.id)}
          <tr class="border-b border-gray-700 hover:bg-neutral-800">
            ...
            <td class="p-3"><button onclick={() => handleDelete(st.id)}
