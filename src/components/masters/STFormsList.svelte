<script lang="ts">
  ...
  let formData = $state({ name: "", registrationType: "Local" as "Local" | "Central" });

  const load = async () => {
    loading = true; error = "";
    try { await companyService.initialize(); stForms = await mastersService.getSTForms(companyId); }
  ...
  const handleSubmit = async () => {
    loading = true; error = "";
    try {
      await mastersService.createSTForm({ companyId, name: formData.name, registrationType: formData.registrationType, description: null });
  ...
  const handleDelete = async (id: number) => {
    ...
    await mastersService.deleteSTForm(id); await load();
  };
...
<div class="p-6">
  ...
  {#if showForm}
    <div class="bg-neutral-800 border border-gray-600 rounded p-4 mb-6 max-w-lg">
      <div class="space-y-3">
        <Input name="name" label="Form Name (e.g. Form C, Form H)" type="text" bind:value={formData.name} required />
        <div>
          <label class="text-sm font-medium block mb-1">Registration Type</label>
          <select bind:value={formData.registrationType} class="w-full p-2 bg-neutral-700 border border-gray-500 rounded">
      ...
      <tbody>
        {#each stForms as f (f.id)}
          <tr class="border-b border-gray-700 hover:bg-neutral-800">
            ...
            <td class="p-3"><button onclick={() => handleDelete(f.id)}
