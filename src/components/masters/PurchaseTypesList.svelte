<script lang="ts">
  ...
  const TAXATION_TYPES: PurchaseType["taxationType"][] = [
    "Taxable",
    "Exempt",
    "Tax Free",
    "Against ST Form",
  ...
  const load = async () => {
    ...
    try {
      ...
      purchaseTypes = await mastersService.getPurchaseTypes(companyId);
    } catch (e) {
  ...
  const handleDelete = async (id: number) => {
    ...
    await mastersService.deletePurchaseType(id);
    await load();
  };
    ...
    >
      <div class="space-y-3">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label for="purchase-region" class="text-sm font-medium block mb-1"
              >Region</label
            >
            <select
              id="purchase-region"
          ...
          <div>
            <label
              for="purchase-taxation"
              class="text-sm font-medium block mb-1">Taxation Type</label
            >
            <select
              id="purchase-taxation"
        ...
        {#if formData.taxationType === "Taxable" || formData.taxationType === "Multi Tax"}
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label
                for="purchase-tax-rate"
                class="text-sm font-medium block mb-1">Tax Rate %</label
              >
              <input
            ...
            <div>
              <label
                for="purchase-surcharge"
                class="text-sm font-medium block mb-1">Surcharge Rate %</label
              >
              <input
                id="purchase-surcharge"
                type="number"
