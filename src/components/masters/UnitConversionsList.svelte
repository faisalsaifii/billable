<script lang="ts">
  ...
  const load = async () => {
    ...
    try {
      ...
      if (units.length) {
        formData.fromUnitId = units[0].id;
        formData.toUnitId = units[0].id;
      }
    } catch (e) {
  ...
  const handleSubmit = async () => {
    ...
    if (formData.conversionFactor <= 0) {
      error = "Conversion factor must be > 0";
      return;
    }
    loading = true;
    error = "";
...
<div class="p-6">
  ...
  {#if showForm && units.length >= 2}
    ...
    >
      ...
      <div class="space-y-3">
        <div>
          <label for="main-unit-from" class="text-sm font-medium block mb-1"
            >Main Unit (from)</label
          >
          <select
            id="main-unit-from"
            bind:value={formData.fromUnitId}
        ...
        <div>
          <label for="sub-unit-to" class="text-sm font-medium block mb-1"
            >Sub Unit (to)</label
          >
          <select
            id="sub-unit-to"
            bind:value={formData.toUnitId}
            class="w-full p-2 bg-neutral-700 border border-gray-500 rounded"
          >
            {#each units as u}<option value={u.id}>{u.name}</option>{/each}
          </select>
        ...
        <div>
          <label for="conversion-factor" class="text-sm font-medium block mb-1"
            >Conversion Factor (1 main = ? sub)</label
          >
          <input
            id="conversion-factor"
            type="number"
            bind:value={formData.conversionFactor}
            min="0.0001"
            step="0.01"
            class="w-full p-2 bg-neutral-700 border border-gray-500 rounded"
          />
