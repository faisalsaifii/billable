...
<div class="p-6 max-w-lg">
  ...
  <p class="text-gray-400 text-sm mb-6">
    Prevent modifications to masters and vouchers on or before the freeze date.
  </p>
  ...
  <div class="bg-neutral-800 border border-gray-600 rounded p-4 space-y-4">
    ...
    {#if freezeData}
      <div>
        <label for="freeze-date" class="block text-sm font-medium mb-2"
          >Freeze Until Date</label
        >
        <input
          id="freeze-date"
          type="date"
          bind:value={freezingDate}
          required
      ...
      <div>
        <label for="freeze-reason" class="block text-sm font-medium mb-2"
        ...
        <textarea
          ...
          rows="3"
          placeholder="e.g., Financial year closure"
          class="w-full p-2 bg-neutral-700 border border-gray-500 rounded text-sm resize-none"
        ></textarea>
      </div>
    {/if}
    ...
    {#if status}
      <div class="bg-green-900 text-green-100 p-3 rounded text-sm">
        {status}
      </div>
    {/if}
  </div>
  ...
  {#if freezeData && freezingDate}
    <div
      class="mt-6 p-4 bg-yellow-900 border border-yellow-600 rounded text-sm text-yellow-100"
    >
      Data is frozen until <strong>{freezingDate}</strong>. Modifications on or
      before this date are not allowed.
    </div>
  {/if}
</div>
